...
export function computeTopFromActions(actions = []) {
  const mapped = actions.map((a) => ({
    ...
    nom: a.Nom,
    coursVeille: Number(a.coursVeille) || 0,
  ...
  return {
    topHausse: hausse.map((x) => ({ ticker: x.ticker, cours: x.coursVeille, change: x.variationP })),
    topBaisse: baisse.map((x) => ({ ticker: x.ticker, cours: x.coursVeille, change: x.variationP })),
